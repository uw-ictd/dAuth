...
class UeransimUe(object):
    def __init__(self, name: str, config: str):
        ...
        self.ue_process_control_port = self._extract_control_port(
            self.process_handle.stderr.readline().decode("utf8")
        )
        ...
        log.info(
            f"Communicating with {name} at port {self.ue_process_control_port} from {self.sock.getsockname()}"
        )
    ...
    def _unpack_message(self, data: bytes) -> str:
        ...
        value_length = struct.unpack("!i", data[8 + name_length : 8 + name_length + 4])[
            0
        ]
        encoded_value = data[8 + name_length + 4 : 8 + name_length + 4 + value_length]
        if (8 + name_length + 4 + value_length) < len(data):
            log.warn(
                "Extra bytes in message: %s",
                data[(8 + name_length + 4 + value_length) :],
            )
