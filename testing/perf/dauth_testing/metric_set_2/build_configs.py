...
def build_configs(config_dir: str, output_dir: str):
    for config_name in os.listdir(config_dir):
        config_content = yaml.safe_load(open(os.path.join(config_dir, config_name), 'r'))
...
def generate_variations(config_name: str, config_content: dict) -> List[Union[str,dict]]:
    # set the home network (offline) to the ueransim node
    config_content["services"][0] = copy.deepcopy(config_content["ueransim"])
    config_content["services"][0]["id"] = \
        config_content["services"][0]["id"].replace("-ueransim", "-service")
