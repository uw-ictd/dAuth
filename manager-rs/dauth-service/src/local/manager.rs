...
/// 2. Generate if 
pub fn auth_vector_get(context: Arc<DauthContext>, av_request: Rc<AuthVectorRequest>)
-> Option<Rc<AuthVectorResult>> {
    println!("local::manager::auth_vector_get");
    if let Some(av_result) = local::database::auth_vector_lookup(context.clone(), av_request.clone()) {
        ...
        Some(av_result)
    } else if let Some(av_result) = remote::manager::auth_vector_send_request(context.clone(), av_request.clone()) {
        Some(av_result)
    } else {
        None
    }
...
pub fn auth_vector_used(context: Arc<DauthContext>, av_result: Rc<AuthVectorResult>)
-> Result<(), &'static str> {
