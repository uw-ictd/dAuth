...
from messaging.messaging_client import GlobalLoggingClient as glc
...
class NextEPCHandler:
    ...
    def __init__(self):
        ...
        self.db = db
        self.client = client
        glc.log('NextEPCHandler', "__init__ -- db is " + str(db.name))
        ...
        triggers.register_op_trigger(self._notify_operation_changes, gvars.db_database_name, DB_COLLECTION_NAME)
        ...
        glc.log('NextEPCHandler', "__init__ -- finished")
    ...
    def _notify_operation_changes(self, op_document):
        glc.log('NextEPCHandler', "_notify_operation_changes -- triggered with " + op_document['op'])
        if op_document['op'] == 'u':
    ...
    def _notify_changes_to_mongo_update(self, op_document):
        ...
        glc.log('NextEPCHandler', "_notify_changes_to_mongo_update -- " + str(serialize_mongodb_cursor(op_document)))
    ...
    def _notify_changes_to_mongo_insert(self, op_document):
        ...
        glc.log('NextEPCHandler', "_notify_changes_to_mongo_update -- " + str(serialize_mongodb_cursor(object_modified)))
        ...
        if 'imsi' in object_dict:
            ...
            for data_collection in cursor:
                if data_collection['imsi'] == object_dict['imsi']:
                    glc.log("NextEPCHandler", "_db_handle_find_and_insert -- imsi exists: " + str(object_dict['imsi']))
                    ...
                    if 'remote' in object_dict:
                        ...
                        glc.log("NextEPCHandler", "_db_handle_find_and_insert -- received from another node, doing nothing")
                    else:
                        ...
                        glc.log("NextEPCHandler", "_db_handle_find_and_insert -- using ccellular client to set")
                        ccellular_client = _get_client(None)
                        ccellular_client.set(object_dict['imsi'], serialized_proto_message)
                else:
                    print("Inserting the value to the {} in collection {}".format(gvars.db_database_name, DB_COLLECTION_NAME))
                    glc.log("NextEPCHandler", "Inserting the value to the {} in collection {}".format(gvars.db_database_name, DB_COLLECTION_NAME))
                    self.db[DB_COLLECTION_NAME].insert_one(object_dict)
        else:
            print("[ERROR] Malformed Request. Cannot process this anymore.")
            glc.log("NextEPCHandler", "_db_handle_find_and_insert -- bad request")

    def _db_handle_update(self, object_queried, modification_to_apply, object_dict):
        glc.log("NextEPCHandler", "_db_handle_update")
    ...
    def _db_handle_delete(self, object_key_id):
        glc.log("NextEPCHandler", "_db_handle_delete")
        query = object_key_id
        cursor = self.db[DB_COLLECTION_NAME].find(query)
        for data_collection in cursor:
