...
#[derive(Debug, PartialEq, Copy, Clone)]
pub struct KasmeShare {
    pub share: [u8; KASME_LENGTH + 1],
}
...
impl TryFrom<Vec<u8>> for KasmeShare {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        res.and_then(|share_bytes| Ok(KasmeShare { share: share_bytes }))
            .or(Err(DauthError::ShamirShareError()))
    }
}

impl TryFrom<&[u8]> for KasmeShare {
    type Error = DauthError;
...
pub fn create_shares_from_kasme<T: rand_0_8::RngCore + std::fmt::Debug>(
    input: &Kasme,
    share_count: u8,
...
pub fn recover_kasme_from_shares(
    input: &Vec<KasmeShare>,
    threshold: u8,
...
mod tests {
    ...
    #[test]
    fn test_vector_creation_deterministic_kasme() {
        let mut rng = rand_0_8::rngs::mock::StepRng::new(1, 1);
        let kasme: Kasme =
            hex::decode("562d716dbd058b475cfecdbb48ed038f562d716dbd058b475cfecdbb48ed038f")
                .unwrap()
                .try_into()
                .unwrap();
        let res = create_shares_from_kasme(&kasme, 5, 3, &mut rng).unwrap();
        println!("{:?}", res);
