...
{
    const ogs_rbnode_t *node = (ogs_rbnode_t *)rb_node;
    ogs_assert(node);
...
#define ogs_typed_rbtree_for_each(tree, node) \
    for (node = (ogs_rbnode_t *)ogs_rbtree_first(tree); \
        (node); node = (ogs_rbnode_t *)ogs_rbtree_next(node))
...
static ogs_inline int ogs_rbtree_count(const ogs_rbtree_t *tree)
{
    ogs_rbnode_t *node;
    int i = 0;
    ogs_typed_rbtree_for_each(tree, node)
        i++;
    return i;
}
